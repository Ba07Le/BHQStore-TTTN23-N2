...
import { useDispatch, useSelector } from 'react-redux'; // 🛠️ Thêm useSelector
import { 
    deleteCartItemByIdAsync, 
    updateCartItemByIdAsync, 
    removeFromGuestCart,      // 🛠️ Import action mới
    updateGuestCartQuantity   // 🛠️ Import action mới
} from '../CartSlice';
import { selectLoggedInUser } from '../../auth/AuthSlice'; // 🛠️ Import selector user
...
export const CartItem = ({id,thumbnail,title,category,brand,price,quantity,stockQuantity,productId}) => {
    ...
    const loggedInUser = useSelector(selectLoggedInUser) // 🛠️ Lấy thông tin user
    ...
    const handleAddQty = () => {
        ...
        if (loggedInUser) {
            ...
            dispatch(updateGuestCartQuantity(update)) // 🛠️ Xử lý cho khách
        }
    }

    const handleRemoveQty = () => {
        if (quantity === 1) {
            handleProductRemove() // Gọi hàm xóa chung bên dưới
        } else {
            const update = { _id: id, quantity: quantity - 1 }
            if (loggedInUser) {
                dispatch(updateCartItemByIdAsync(update))
